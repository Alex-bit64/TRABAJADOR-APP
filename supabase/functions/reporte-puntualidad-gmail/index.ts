...
async function obtenerHorarios(dnis: string[], diaSemana: string) {
  const { data, error } = await supabase
    ...
    .select("dni_trabajador,dia_semana,horario_entrada,horario_salida")
    .eq("dia_semana", diaSemana)
    .in("dni_trabajador", dnis);
  if (error) {
    throw new Error(`No se pudo leer horario_trabajador: ${error.message}`);
  }
  return ((data ?? []) as Horario[]).filter((horario) => {
    if (!horario.horario_entrada || !horario.horario_salida) {
      return false;
    }
    const entrada = minutosDesdeHora(horario.horario_entrada);
    const salida = minutosDesdeHora(horario.horario_salida);
    return entrada > 0 && salida > 0 && entrada < salida;
  });
}
